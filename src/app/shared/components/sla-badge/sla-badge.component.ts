  ...
  template: `
    <div class="flex items-center gap-2 group relative cursor-help">
      <div [ngClass]="{
            ...
            'bg-surface-900 text-white border border-surface-700': sla().status === 'EXPIRED'
        }">
      ...
      <!-- Tooltip -->
      <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 p-2 bg-surface-900 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity invisible group-hover:visible z-10 shadow-xl">
        <span class="text-xs font-semibold">SLA Status: {{ sla().status }}</span>
        <div class="w-full bg-white/20 h-1 rounded-full mt-1">
